package Gongbok_BE.Gongbok.auth.jwt.service;
...
public class JwtService {
    ...
    public void updateRefreshToken(String email, String refreshToken) {
        memberRepository.findByEmail(email)
                .ifPresentOrElse(
                        client -> {
                            client.updateRefreshToken(refreshToken);
                            memberRepository.saveAndFlush(client);
                            // redis에 저장
                            redisTemplate.opsForValue().set(email, refreshToken, refreshTokenExpirationPeriod, TimeUnit.SECONDS);
                        },
                        () -> {
                            throw new AllGongbokException(ErrorCode.MEMBER_NOT_FOUND, ErrorCode.MEMBER_NOT_FOUND.getMessage());
                        }
                );
    }
...
//        else throw new AllGlinsException(ErrorCode.INVALID_TOKEN_LOGOUT, ErrorCode.INVALID_TOKEN_LOGOUT.getMessage());
    ...
    public MemberResponseDto.logout logout(){
        ...
        if (!this.isTokenValid(accessToken)) {
            throw new AllGongbokException(ErrorCode.INVALID_TOKEN, ErrorCode.INVALID_TOKEN.getMessage());
        }
